...
export function decodePng(file: Uint8Array): Rgba {
  ...
  while (off < file.length) {
    ...
    if (type === 'IHDR') {
      ...
      width = h.getUint32(0); height = h.getUint32(4);
...
export function encodePng(img: Rgba): Uint8Array {
  ...
  h.setUint32(0, img.width); h.setUint32(4, img.height);
  ...
  return Buffer.concat([Uint8Array.from(SIG), chunk('IHDR', ihdr), chunk('IDAT', deflateSync(raw)), chunk('IEND', new Uint8Array(0))]);
}
