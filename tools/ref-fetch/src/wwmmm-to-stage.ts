...
import {
  BALL_RADIUS_M, LEVEL_HEIGHT_M, MIN_BRIDGE_WIDTH_PX, PX_PER_METER,
  type Bridge, type Elevator, type Island, type Item, type StageData, type Vec2,
} from './stage-types.ts';

export interface WwmmmIsland { id: number; level: number; contours: number[][]; guardrails: number[][]; restartPoints: number[] }
export interface WwmmmBridge { angle: number; distance: number; level: [number, number]; start: [number, number]; type: number; width: number }
export interface WwmmmStage {
  title: string; url: string; image?: string; mobile_image?: string;
  start: [number, number, number]; goal: [number, number, number];
  islands: WwmmmIsland[]; bridges: WwmmmBridge[];
  small_items: number[]; large_items: number[];
}
...
const pairs = (a: number[]): Vec2[] => { const o: Vec2[] = []; for (let i = 0; i + 1 < a.length; i += 2) o.push([a[i], a[i + 1]]); return o; };
const triples = (a: number[]): [number, number, number][] => { const o: [number, number, number][] = []; for (let i = 0; i + 2 < a.length; i += 3) o.push([a[i], a[i + 1], a[i + 2]]); return o; };
...
export function convertWwmmm(src: WwmmmStage, opts: ConvertOptions): ConvertResult {
  ...
  src.bridges.forEach((b, idx) => {
    ...
    const levelA = quantize(b.level[0]), levelB = quantize(b.level[1]);
    if (b.type === 0) {
      bridges.push({ id: bridges.length, from, to, a: S(a), b: S(e), width: r2(b.width * k), type: levelA === levelB ? 'flat' : 'ramp', levelA, levelB });
    } else if (b.type === 1) {
      ...
      elevators.push({
        id: elevators.length, islandFrom: from, islandTo: to,
        pos: S([a[0] + dir[0] * mid, a[1] + dir[1] * mid]),
        size: r2(Math.max(len, b.width) * k),
        levelLow: Math.min(levelA, levelB), levelHigh: Math.max(levelA, levelB),
  ...
  const items: Item[] = [];
  for (const [kind, arr] of [['small', src.small_items], ['large', src.large_items]] as const) {
    for (const t of triples(arr)) items.push({ id: items.length, kind, pos: S([t[0], t[1]]), islandId: islandAt([t[0], t[1]]) });
  }

  const pageWidth = r2(ORIGINAL.STAGE_WIDTH_PX * k), pageHeight = r2(ORIGINAL.STAGE_HEIGHT_PX * k);
  ...
  const stageId = createHash('sha256').update(captureId + seed + builderVersion + difficulty).digest('hex');
  ...
  const stage: StageData = {
    schema: 'wwm.stage/1',
    stageId, builderVersion, seed, difficulty,
    ...
    islands, bridges, elevators, items,
    start: { pos: S([src.start[0], src.start[1]]), islandId: islandAt([src.start[0], src.start[1]]) },
    goal: { pos: S([src.goal[0], src.goal[1]]), islandId: islandAt([src.goal[0], src.goal[1]]), radius: r2(ORIGINAL.SENSOR_RADIUS_PX * k) },
    provenance: {
      keptElementIds: [],
      dropped: [],
      notes: [
  ...
  const extra = {
    ...
    counts: { islands: src.islands.length, bridges: src.bridges.filter((b) => b.type === 0).length, elevators: src.bridges.filter((b) => b.type === 1).length, smallItems: src.small_items.length / 3, largeItems: src.large_items.length / 3 },
  };
...
export function checkStructure(s: StageData): string[] {
  ...
  for (const i of s.islands) {
    ...
    if (signedArea(i.contour) > 0) issues.push(`island ${i.id}: outer ring has positive signed area in page coords (y-down)`);
    ...
    for (const p of i.restartPoints) {
      ...
      else if (distToRing(p, i.contour) < edgeMin) issues.push(`island ${i.id}: restart point ${distToRing(p, i.contour).toFixed(1)}px from edge (< ${edgeMin})`);
    }
  }
  for (const b of s.bridges) {
    ...
    if (b.width < MIN_BRIDGE_WIDTH_PX) issues.push(`bridge ${b.id}: width ${b.width} < ${MIN_BRIDGE_WIDTH_PX}`);
    const d = Math.abs(b.levelA - b.levelB);
    if (b.type === 'ramp' && d !== 1) issues.push(`bridge ${b.id}: ramp level difference ${d} (contract requires exactly 1)`);
    if (b.type === 'flat' && d !== 0) issues.push(`bridge ${b.id}: flat bridge with level difference ${d}`);
    const fa = byId.get(b.from), fb = byId.get(b.to);
    if (fa && b.levelA !== fa.level) issues.push(`bridge ${b.id}: levelA ${b.levelA} != island ${b.from} level ${fa.level}`);
    if (fb && b.levelB !== fb.level) issues.push(`bridge ${b.id}: levelB ${b.levelB} != island ${b.to} level ${fb.level}`);
    for (let t = 0.1; t < 0.9; t += 0.1) {
      const p: Vec2 = [b.a[0] + (b.b[0] - b.a[0]) * t, b.a[1] + (b.b[1] - b.a[1]) * t];
      for (const i of s.islands) {
        if (i.id !== b.from && i.id !== b.to && pointInPolygon(p, i.contour)) { issues.push(`bridge ${b.id}: crosses island ${i.id}`); t = 1; break; }
      }
    }
  }
  for (const e of s.elevators) if (!byId.has(e.islandFrom) || !byId.has(e.islandTo)) issues.push(`elevator ${e.id}: unknown island`);
  for (const it of s.items) {
    const isl = byId.get(it.islandId);
    if (!isl) { issues.push(`item ${it.id}: unknown island`); continue; }
    if (!pointInPolygon(it.pos, isl.contour)) issues.push(`item ${it.id} (${it.kind}): outside island ${it.islandId}`);
    else if (distToRing(it.pos, isl.contour) < edgeMin) issues.push(`item ${it.id} (${it.kind}): ${distToRing(it.pos, isl.contour).toFixed(1)}px from edge (< ${edgeMin})`);
  }
  // Reachability from the start island through bridges and elevators (undirected).
  const adj = new Map<number, number[]>();
  const link = (x: number, y: number): void => { adj.set(x, [...(adj.get(x) ?? []), y]); adj.set(y, [...(adj.get(y) ?? []), x]); };
  ...
  while (queue.length) for (const n of adj.get(queue.shift() as number) ?? []) if (!seen.has(n)) { seen.add(n); queue.push(n); }
  const unreachable = s.islands.filter((i) => !seen.has(i.id)).map((i) => i.id);
  if (unreachable.length) issues.push(`unreachable islands from start island ${s.start.islandId}: ${unreachable.join(',')}`);
...
export function convertFiles(args: { jsonPath: string; pngPath: string; outDir: string; slug: string; scale?: ConvertOptions['scale'] }): ConvertResult {
  ...
  const res = convertWwmmm(src, { scale: args.scale ?? 'ball', texturePath: texFile, texture: { width: tex.width, height: tex.height } });
  ...
  const report = [
    ...
    '', '## Issues by kind', ...summarise(res.issues),
    '', '## Unmapped / lossy fields', ...res.unmapped.map((u) => `- ${u}`),
    '', '## All issues', ...res.issues,
  ].join('\n');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (si >= 0) { const v = argv[si + 1]; scale = v === 'raw' || v === 'ball' ? v : Number(v); argv.splice(si, 2); }
