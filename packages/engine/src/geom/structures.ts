 ...
 */
import { LEVEL_HEIGHT_M, mulberry32, PX_PER_METER, type StageData, signedArea, type Vec2 } from '@wwm/schema';
import { ShapeUtils, Vector2 } from 'three/webgpu';
import { BRIDGE_THICKNESS_M, HSV, ISLAND_THICKNESS_M, RAIL_HEIGHT_M } from '../palette.ts';
...
const RAIL_BAR = 0.075;
const RAIL_POST = 0.06;
const POST_SPACING_M = 1.9;
/** Bridges poke this far into their islands so there is never a hairline gap (px). */
const BRIDGE_OVERLAP_PX = 3;
...
export function triangulateIsland(contour: readonly Vec2[], holes: readonly (readonly Vec2[])[]): [Vec2, Vec2, Vec2][] {
...
export function buildStageMeshes(stage: StageData, plan: TilePlan): StageMeshes {
  ...
  for (const br of stage.bridges) {
    ...
    const a: Vec2 = [br.a[0] - d[0] * BRIDGE_OVERLAP_PX, br.a[1] - d[1] * BRIDGE_OVERLAP_PX];
    const b: Vec2 = [br.b[0] + d[0] * BRIDGE_OVERLAP_PX, br.b[1] + d[1] * BRIDGE_OVERLAP_PX];
    const ya = toY(br.levelA);
    const yb = toY(br.levelB);
    const heightAt = (t: number) => {
      // t over the extended deck; the overlap stays level with its island
      const tt = Math.min(1, Math.max(0, (t * (len + 2 * BRIDGE_OVERLAP_PX) - BRIDGE_OVERLAP_PX) / len));
      return ya + (yb - ya) * tt;
    };
    deck(bridges, rng, a, b, br.width, heightAt, BRIDGE_THICKNESS_M);
    // Side rails on both edges (E: "a thin side rail each side").
    rails.face.group = GROUP_BRIDGE;
    rails.face.delay = delay;
    rails.face.hsv = HSV.yellow;
    const n: Vec2 = [-d[1], d[0]];
    const inset = br.width / 2 - 2;
    for (const side of [-1, 1]) {
      const pts: V3[] = [];
      const segs = Math.max(1, Math.ceil(toX(len) / SEGMENT_M));
      for (let s = 0; s <= segs; s++) {
        const t = s / segs;
        const px = br.a[0] + (br.b[0] - br.a[0]) * t + n[0] * inset * side;
        const py = br.a[1] + (br.b[1] - br.a[1]) * t + n[1] * inset * side;
  ...
  // ── Elevator shafts (static): red corner pillars from the lower to above the upper level ──
  for (const el of stage.elevators) {
    ...
    const fp = elevatorFootprint(el.a, el.b, el.width);
    const y0 = toY(el.levelLow) - ISLAND_THICKNESS_M;
    const y1 = toY(el.levelHigh) + RAIL_HEIGHT_M + 0.2;
    rails.face.base = y0;
    for (const c of fp) box(rails, [toX(c[0]), y0, toX(c[1])], [toX(c[0]), y1, toX(c[1])], 0.09);
  }
  ...
  for (const isl of stage.islands) {
    ...
    for (const line of isl.guardrails) {
      ...
      railAlong(
        rails,
        line.map((p) => [toX(p[0]), y, toX(p[1])] as V3),
        RAIL_HEIGHT_M,
      );
    }
  }
...
/**
 * Elevator platform footprint (px): centred between a and b, length max(|b − a|, 15 px) along a→b
 * (E: `elevator` platforms are max(distance, 15) px long), full width across. Perimeter order.
 */
export function elevatorFootprint(a: Vec2, b: Vec2, width: number): Vec2[] {
  const dx = b[0] - a[0];
  const dy = b[1] - a[1];
  const len = Math.hypot(dx, dy);
  const d: Vec2 = len > 1e-6 ? [dx / len, dy / len] : [1, 0];
  const n: Vec2 = [-d[1], d[0]];
  const L = Math.max(len, 15) / 2;
  const W = width / 2;
  const c: Vec2 = [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2];
  return [
...
function deck(
  ...
  const w = widthPx / 2;
  const segs = Math.max(1, Math.ceil(toX(len) / SEGMENT_M));
  const P = (t: number, s: number): Vec2 => [a[0] + (b[0] - a[0]) * t + n[0] * w * s, a[1] + (b[1] - a[1]) * t + n[1] * w * s];
  for (let i = 0; i < segs; i++) {
    ...
    if (i === 0) mb.quad(up(L0, h0), up(R0, h0), dn(R0, h0), dn(L0, h0), [2, 0], [2, 0], [2, 0], [2, 0], [-d[0], 0, -d[1]]);
    if (i === segs - 1) mb.quad(up(L1, h1), up(R1, h1), dn(R1, h1), dn(L1, h1), [2, 0], [2, 0], [2, 0], [2, 0], [d[0], 0, d[1]]);
  }
}
...
export function railAlong(mb: MeshBuilder, pts: readonly V3[], height: number): void {
  ...
  for (let i = 0; i + 1 < pts.length; i++) {
    ...
    box(mb, [p[0], p[1] + height, p[2]], [q[0], q[1] + height, q[2]], RAIL_BAR, true);
    // posts
