...
- **Ocean** (E concept, new art): a 2.8 km faceted plane of the 8 `COLOR_TRIANGLE` pastels, 194 m below the lowest island. It has in-shader `COLOR_WIRE` wire lines and vertex dots (so there is no z-fighting overlay), a slow swell, a double ripple ring on ball loss, 1400 floating "star" dots (skipped when their slab is outside the view frustum), and a ring of 30 procedural clouds on the horizon. It **leans with the tilt** around the ball (E; off under reduced motion).
