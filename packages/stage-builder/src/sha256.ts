/**
 * Synchronous SHA-256 (UTF-8 input, lower-case hex output). `buildStage` is synchronous, so it can't use
 * `crypto.subtle`; this produces exactly the same digest as `@wwm/schema`'s `sha256Hex` (tested).
 */
