 ...
 * Phase 03's `@wwm/stage-builder` is not merged yet, so `defaultStageBuilder()` returns STUB_BUILDER. The
 * orchestrator wires the real one at merge by changing only `defaultStageBuilder` (see README).
 */
import {
  type BuildInput,
  type BuildResult,
...
/**
 * The builder the Worker uses. ORCHESTRATOR: at Phase 03 merge replace the body with
 *   `return { version: STAGE_BUILDER_VERSION, buildStage };` (imports from '@wwm/stage-builder').
 */
