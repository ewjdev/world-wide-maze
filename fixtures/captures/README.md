...
These 6 fixtures are **legacy DPR 1 captures** (`screenshot.scale: 1`, added in contract v0.2 without recapturing).
...
- Phase 02 originally had a 7th fixture, a news-site card grid. Its photos and text belonged to the news organisation and its agencies, so it was kept internal and then removed before the repo was published (`govuk-card-grid` covers the same card-grid role). See `NOTICE.md`. The eval pages added by Phase 09 (`eval-*`) are listed with their sources and licences in `tools/batch-eval/eval-captures.json`.
